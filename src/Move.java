...
class Move {
    ...
    public int find(String itemName) {
        for (int i = 0; i < boxes.size(); i++) {
            if (findItemInBox(boxes.get(i), itemName)) {
                return boxes.get(i).getBoxNumber();
            }
        }
        return -1;
    }
    ...
    private boolean findItemInBox(Box box, String itemName) {
        for (Object item : box.getItems()) {
            if (item instanceof SingleObject singleObject) {
                if (singleObject.getItemName().equals(itemName)) {
                    System.out.println(singleObject.getItemName());
                    return true;
                }
    ...
    private void printObjects(List<Object> items) {
        for (Object item : items) {
            if (item instanceof SingleObject singleObject) {
                System.out.println(singleObject.getItemName());
            } else if (item instanceof Box) {
                printObjects(((Box) item).getItems());
            }
        }
    }
